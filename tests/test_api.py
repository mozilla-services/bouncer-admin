...
def test_location_show_exact_match(client):
    rv = client.get("/api/location_show/?product=Firefox", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
...
def test_location_show_exact_no_match(client):
    rv = client.get("/api/location_show/?product=NoMatch", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
    expected = b'<?xml version="1.0" encoding="utf-8"?>'
    assert expected == rv.data


def test_location_show_fuzzy_match(client):
    rv = client.get("/api/location_show/?product=Firefox&fuzzy=True", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
...
def test_location_show_fuzzy_no_match(client):
    rv = client.get("/api/location_show/?product=NoMatch&fuzzy=True", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
    expected = b'<?xml version="1.0" encoding="utf-8"?>'
    assert expected == rv.data


def test_uptake(client):
    rv = client.get("api/uptake/?product=Firefox&os=osx&fuzzy=True", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
...
def test_mirror_list(client):
    rv = client.get("api/mirror_list/", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
...
def test_location_add_os_not_found(client):
    rv = client.post(
        "api/location_add/", data=dict(product="Firefox", path="/test_path", os="fake"), headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)}
    )
...
def test_location_add_location_exists(client):
    rv = client.post(
        "api/location_add/", data=dict(product="Firefox", path="/test_path", os="osx"), headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)}
    )
...
def test_location_delete_location_not_found(client):
    rv = client.post("api/location_delete/", data=dict(location_id=0), headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
...
def test_location_delete_location_input_missing(client):
    rv = client.post("api/location_delete/", data=dict(not_loc_id="fakeloc"), headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
    expected = b'<?xml version="1.0" encoding="utf-8"?><error number="101">location_id is required.</error>'
    assert expected == rv.data


def test_location_delete(client):
    rv = client.post("api/location_delete/", data=dict(location_id=23194), headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
...
def test_product_show_exact_match(client):
    rv = client.get("/api/product_show/?product=Firefox", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
...
def test_product_show_exact_no_match(client):
    rv = client.get("/api/product_show/?product=NoMatch", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
    expected = b'<?xml version="1.0" encoding="utf-8"?><products/>'
    assert expected == rv.data


def test_product_show_fuzzy_match(client):
    rv = client.get("/api/product_show/?product=Firefox&fuzzy=True", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
...
def test_product_show_fuzzy_no_match(client):
    rv = client.get("/api/product_show/?product=NoMatch&fuzzy=True", headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
    expected = b'<?xml version="1.0" encoding="utf-8"?><products/>'
    assert expected == rv.data
...
def test_product_delete_id(client):
    rv = client.post("api/product_delete/", data={"product_id": 4556}, headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
...
def test_product_delete_name_no_match(client):
    rv = client.post("api/product_delete/", data={"product": "Fake"}, headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
    expected = b'<?xml version="1.0" encoding="utf-8"?><error number="102">No product found.</error>'
    assert expected == rv.data


def test_product_delete_name(client):
    rv = client.post("api/product_delete/", data={"product": "AaronProduct"}, headers={"Authorization": requests.auth._basic_auth_str(test_user, test_pass)})
    expected = b'<?xml version="1.0" encoding="utf-8"?><success>SUCCESS: product has been deleted</success>'
    msm._reset_db()
    assert expected == rv.data

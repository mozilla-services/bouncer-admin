...
@click.option('-langs', '--languages', default=None, help='list of languages separated by commas (eg. \'en-US,es-SP,fr-FR\' )')
...
@click.option('-h', '--help_method', default=False, is_flag=True, help='get usage on given method')
def main(method, product_id, product, fuzzy, os, path, languages, ssl_only, location_id, alias, related_product, help_method):
    if help_method:
        print(usage(method))
        return
    err = False
    if method == "mirror_list":
        ...
        if not (product and os and path):
            resp = "ERROR: product name required for location modify -- " + usage(method)
            err = True
        else:
        ...
        if not (product and languages):
            resp = "ERROR: product name and languages required for product add -- " + usage(method)
            err = True
        else:
        ...
        if not (product or product_id):
            resp = "ERROR: product name or product_id required for product delete -- " + usage(method)
            err = True
        else:
        ...
        if not (product and languages):
            resp = "ERROR: product name and languages required for product language add -- " + usage(method)
            err = True
        else:
        ...
        if not (product and languages):
            resp = "ERROR: product name and languages required for product language delete -- " + usage(method)
            err = True
        else:
        ...
        if not (alias and related_product):
            resp = "ERROR: alias and related_product required for create update alias -- " + usage(method)
            err = True
        else:
...
def usage(method):
    if method == "mirror_list":
        ...
        return "USAGE: python3 cli.py create_update_alias -a [ALIAS] -rp [RELATED PRODUCT]"
...
def location_show(product, fuzzy):
    http = urllib3.PoolManager()
    r = http.request('GET', base_url + '/api/location_show/?product=' + product + '&fuzzy=' + str(fuzzy))
    return r.data
...
def location_add(product, os, path):
    ...
    r = http.request('POST', base_url + '/api/location_add/',
                 headers=headers,
                 body=data)
...
def location_modify(product, os, path):
    ...
    headers = urllib3.util.make_headers(basic_auth=username + ':' + password)
    headers["Content-Type"] = "application/x-www-form-urlencoded"
    r = http.request('POST', base_url + '/api/location_modify/',
                 headers=headers,
                 body=data)
...
def location_delete(location_id):
    ...
    headers = urllib3.util.make_headers(basic_auth=username + ':' + password)
    headers["Content-Type"] = "application/x-www-form-urlencoded"
    r = http.request('POST', base_url + '/api/location_delete/',
                 headers=headers,
                 body=data)
...
def product_show(product, fuzzy):
    http = urllib3.PoolManager()
    r = http.request('GET', base_url + '/api/product_show/?product=' + product + '&fuzzy=' + str(fuzzy))
    return r.data
...
def product_add(product, languages, ssl_only):
    ...
    headers = urllib3.util.make_headers(basic_auth=username + ':' + password)
    headers["Content-Type"] = "application/x-www-form-urlencoded"
    r = http.request('POST', base_url + '/api/product_add/',
                 headers=headers,
                 body=data)
...
def product_delete(product, product_id):
    ...
    headers = urllib3.util.make_headers(basic_auth=username + ':' + password)
    headers["Content-Type"] = "application/x-www-form-urlencoded"
    r = http.request('POST', base_url + '/api/product_delete/',
                 headers=headers,
                 body=data)
...
def product_language_add(product, languages):
    ...
    headers = urllib3.util.make_headers(basic_auth=username + ':' + password)
    headers["Content-Type"] = "application/x-www-form-urlencoded"
    r = http.request('POST', base_url + '/api/product_language_add/',
                 headers=headers,
                 body=data)
...
def product_language_delete(product, languages):
    ...
    headers = urllib3.util.make_headers(basic_auth=username + ':' + password)
    headers["Content-Type"] = "application/x-www-form-urlencoded"
    r = http.request('POST', base_url + '/api/product_language_delete/',
                 headers=headers,
                 body=data)
...
def uptake(product, os, fuzzy):
    http = urllib3.PoolManager()
    r = http.request('GET', base_url + '/api/uptake/?product=' + product + '&os=' + os + '&fuzzy=' + str(fuzzy))
    return r.data
...
def create_update_alias(alias, related_product):
    ...
    headers = urllib3.util.make_headers(basic_auth=username + ':' + password)
    headers["Content-Type"] = "application/x-www-form-urlencoded"
    r = http.request('POST', base_url + '/api/create_update_alias/',
                 headers=headers,
                 body=data)

...
class MySQLModel:
    ...
    def location_add(self, product, os, path):
        ...
        if not (product and os and path):
            return xml.error(
                'product, os, and path are required POST parameters.',
                errno=101), False
        ...
        if (not os_exists):
            return xml.error('FAILED: \'' + os + '\' does not exist',
                             errno=104), False
        if (not product_exists):
            return xml.error('FAILED: \'' + product + '\' does not exist',
                             errno=104), False
        if (location_exists):
    ...
    def location_modify(self, product, os, path):
        ...
        if (not location_exists):
            return xml.error('FAILED: location \'' + product + '\' on OS \'' +
                             os + '\' does not exist',
                             errno=104), False
    ...
    def create_update_alias(self, alias, related_product):
        ...
        if not related_product:
            return xml.error('Related product name not provided',
                             errno=103), False
